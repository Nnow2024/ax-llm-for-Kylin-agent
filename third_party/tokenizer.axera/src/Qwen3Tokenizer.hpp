...
template <bool ForceNoThink = false, ContentType... Types>
...
public:
    std::string apply_chat_template(const std::vector<Content> &contents, bool add_generation_prompt) override
    {
        ...
        {
            text << "<|im_start|>assistant\n";
            if (ForceNoThink && !this->think_in_prompt)
            {
                // Qwen3 hard-disables thinking when the assistant prefix already contains an empty think block.
                text << "<think>\n\n</think>\n\n";
            }
        }
...
using qwen3vl_tokenizer = Qwen3Tokenizer<true, TEXT, IMAGE, VIDEO>;
REGISTER(Qwen3VL, qwen3vl_tokenizer)
using qwen3_tokenizer = Qwen3Tokenizer<true, TEXT>;
REGISTER(Qwen3, qwen3_tokenizer)
using qwen2_5_tokenizer = Qwen3Tokenizer<false, TEXT>;
REGISTER(Qwen2_5, qwen2_5_tokenizer)
using qwen3omni_tokenizer = Qwen3Tokenizer<true, TEXT, IMAGE, VIDEO, AUDIO>;
REGISTER(Qwen3Omni, qwen3omni_tokenizer)
using qwen3_5_tokenizer = Qwen3Tokenizer<true, TEXT>;
REGISTER(Qwen3_5, qwen3_5_tokenizer)
using qwen3_5vl_tokenizer = Qwen3Tokenizer<true, TEXT, IMAGE, VIDEO>;
