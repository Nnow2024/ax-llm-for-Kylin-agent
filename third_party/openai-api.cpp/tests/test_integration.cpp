...
void test_end_to_end() {
    ...
    std::thread producer([provider]() {
        ...
        auto final_chunk = OutputChunk::TextDelta("World", "gpt-4");
        final_chunk.obj["finish_reason"] = "stop";
        provider->push(std::move(final_chunk));
        provider->end();
    });
    ...
    assert(deltas.back().find("\"content\":\"World\"") != std::string::npos);
    assert(deltas.back().find("\"finish_reason\":\"stop\"") != std::string::npos);
