...
void test_chat_sse_encoder() {
    ...
    auto final_delta = OutputChunk::TextDelta(" World", "gpt-4");
    final_delta.id = "test-id";
    final_delta.obj["finish_reason"] = "stop";
    nlohmann::json final_delta_json = nlohmann::json::parse(encoder.encode(final_delta).substr(6));
    assert(final_delta_json["choices"][0]["delta"]["content"] == " World");
    assert(final_delta_json["choices"][0]["finish_reason"] == "stop");
